...
int	parent_process(pid_t pid, int *pipe_fd, int *file_fd)
{
	int		status;
	pid_t	wait_pid;
...
int main_cmds_with_fork(char ***cmds, t_list *env, int *pipe_fd, int *file_fd)
{
	int		redirection;
	char	**cmd;
	...
	if (!(exec_cmds_with_fork(cmd, env)))
	{
		close(*file_fd);
		free_2d_array(cmd);
		return(0);
...
int process_cmds_with_fork(char ***cmds, t_list *env)
{
	...
	else if (pid == 0)
	{
		...
		printf("====================\n");
		file_fd = 0;
		if (!main_cmds_with_fork(cmds, env, pipe_fd, &file_fd))
			exit(1);
		sleep(1);
		exit(0);
	}
	...
	{	
		if (!parent_process(pid, pipe_fd, &file_fd))
			return (0);
	}
	return (1);
}
...
int process_cmd(char ***cmds, t_list *env)
{
	...
	while (*cmds)
	{
		...
		if (!check_fd_aggregation(*cmds))
			return (0);
		if ((ret = exec_cmds_without_fork(*cmds, env)) == -1)
			return (0);	
		else if (ret == 0)
		{
